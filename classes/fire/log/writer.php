...
class Fire_Log_Writer extends Log_Writer {
	...
	public function __construct(array $options = array())
	{
		...
		$this->_log_cookies = Arr::get($options, 'cookies', FALSE);
	...
	public function __destruct()
	{
		...
		// Log the current session by default?
		if ($this->_log_cookies === TRUE)
		{
			$this->log_cookies($_COOKIE);
		}
	}
	...
	public function log_session(Session $session)
	{
		$table   = array();
		$table[] = array('','');
		$table[] = array('Session Data', $session->as_array());
		$this->_fire->table('Session', $table);
	}
	
	/**
	 * Logs the cookies
	 * 
	 * @param	Cookies	session to log
	 * @return	void
	 */
	public function log_cookies($cookies)
	{
		$table   = array();
		$table[] = array('','');
		$table[] = array('Cookies', $cookies);
		$this->_fire->table('Cookies', $table);
